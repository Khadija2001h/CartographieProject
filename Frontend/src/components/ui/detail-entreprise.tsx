...
export function EntrepriseDetails(): React.JSX.Element {
  ...
  const handleNavigateToGoogleMaps = (latitude: string, longitude: string) => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition((position) => {
        const currentLat = position.coords.latitude;
        const currentLng = position.coords.longitude;
        const url = `https://www.google.com/maps/dir/?api=1&origin=${currentLat},${currentLng}&destination=${latitude},${longitude}`;
        window.open(url, '_blank');
      }, (error) => {
        console.error("Erreur de géolocalisation:", error);
        alert("Erreur lors de la récupération de la position actuelle. Veuillez vérifier vos paramètres de localisation.");
      });
    } else {
  ...
  const defaultLogo = '/assets/logo_par_default.png';
                ...
                <Marker position={[parseFloat(entreprise.latitude!), parseFloat(entreprise.longitude!)]}>
                  <Popup>
                    ...
                    <Button
                      variant="text"
                      color="primary"
                      onClick={() => handleNavigateToGoogleMaps(entreprise.latitude!, entreprise.longitude!)}
                    >
                      Voir l'itinéraire
                    </Button>
                  </Popup>
